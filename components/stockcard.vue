    ...
    <v-card elevation="5">
        <v-card-title>
            <v-row>
                ...
                <v-col cols="5"
                    v-if="cardStyle != 'mini'">
                    <v-switch :label="`切換至${idOdd ? '整股' : '零股'}`"
                        color="indigo"
                        :model-value="idOdd"
                        inset
                        hide-details
        ...
        <v-row style="height: 350px;padding: 0;margin: 0;"
            v-if="!idOdd && cardStyle != 'nochart' && cardStyle != 'mini'">
            <ClientOnly>
                <stockchart v-if="symbol"
                    :symbol="symbol"
...
const emit = defineEmits(['targetOdd'])
function targetOdd(code: string) {
    emit('targetOdd', code)
}
</script>
